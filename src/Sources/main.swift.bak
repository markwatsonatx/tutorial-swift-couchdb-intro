...
router.get("/couch/createdb") { _, response, next in
    Log.info("Request for /couch/createdb")
    response.headers["Content-Type"] = "text/plain; charset=utf-8"
    couch.createDb(db: "test") { (couchResponse, error) in
        if (couchResponse != nil) {
            do {
                try response.send("\(couchResponse!.ok)").end()
            }
            catch {}
        }
        else {
...
router.get("/couch/createdoc") { _, response, next in
    Log.info("Request for /couch/createdoc")
    response.headers["Content-Type"] = "text/plain; charset=utf-8"
    couch.createDoc(db: "test", doc: ["animal":"tiger","fruit":"banana"]) { (couchResponse, error) in
        if (couchResponse != nil) {
            do {
                try response.send("\(couchResponse!.ok)").end()
            }
...
router.get("/couch/alldocs") { _, response, next in
    Log.info("Request for /couch/alldocs")
    response.headers["Content-Type"] = "text/plain; charset=utf-8"
    couch.getAllDocs(db: "test") { (rows, error) in
        if (rows != nil) {
            do {
                try response.send("\(rows!.count) Rows").end()
            }
            catch {}
        }
        else {
            do {
                try response.send("No Rows").end()
            }
            catch {}
        }
    }
...
router.get("/couch/changes") { _, response, next in
    Log.info("Request for /couch/changes")
    response.headers["Content-Type"] = "text/plain; charset=utf-8"
    couch.getChanges(db: "test", since: nil, includeDocs: false) { (changes, error) in
        if (changes != nil) {
            do {
                try response.send("\(changes!.rows.count) Rows").end()
            }
            catch {}
        }
        else {
